...
def test_timeout_cleanup_failure_marks_job_failed(tmp_path):
    helper = tmp_path / "worker.py"
    helper.write_text("import time\nwhile True: time.sleep(1)\n")
    fake_bash_env = tmp_path / "bash_env"
    fake_bash_env.write_text(
        "kill() {\n"
    ...
    process = subprocess.Popen(
        [str(script_path), "0", "config.pkl", "10", "1"],
        cwd=str(tmp_path),
        env={**os.environ, "_CONDOR_SCRATCH_DIR": str(tmp_path), "BASH_ENV": str(fake_bash_env)},
        start_new_session=True,
    )
    time.sleep(0.5)
    process.send_signal(signal.SIGTERM)
